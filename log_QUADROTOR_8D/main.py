...
def loss_pos_matrix_eigen_values(A):
    ...
    upper = True
    eigv = torch.linalg.eigh(A, UPLO='U' if upper else 'L')[0].view(-1)
...
def forward(x, xref, uref, _lambda, verbose=False, acc=False, detach=False):
    ...
    if verbose:
        eigv = torch.linalg.eigh(Contraction, UPLO='U')[0]
        print(eigv.min(dim=1)[0].mean(), eigv.max(dim=1)[0].mean(), eigv.mean())

    if acc:
        eigv_Contraction = torch.linalg.eigh(Contraction, UPLO='U')[0]
        eigv_C1 = torch.linalg.eigh(C1_LHS_1, UPLO='U')[0]
        return (
            loss,
            ((eigv_Contraction >= 0).sum(dim=1) == 0).cpu().detach().numpy(),
            ((eigv_C1 >= 0).sum(dim=1) == 0).cpu().detach().numpy(),
            sum([(C2**2).reshape(bs, -1).sum(dim=1).mean().item() for C2 in C2s])
        )
    else:
        return loss, None, None, None
